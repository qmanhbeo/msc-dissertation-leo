"""
Preprocess SDGi Corpus for multi-label SDG classification.

Input:  2_data/0_raw/sdgi_corpus/sdgi_corpus.parquet  (5,880 rows, multi-label)
Output: 2_data/1a_preprocessed_multilabel/sdgi_corpus/sdgi_clean.jsonl
        2_data/1a_preprocessed_multilabel/sdgi_corpus/sdgi_clean.csv
...
Role in pipeline:
  Supervised multi-label counterpart — provides policy-domain (VNR/VLR) SDG reference
  with full multi-label signal preserved. Output uses "sdgs" (list of ints) field.

Run from project root:
    python 1_code/1a_preprocess_multilabel/preprocess_sdgi_corpus.py
"""
...
OUTPUT_JSONL = Path("2_data/1a_preprocessed_multilabel/sdgi_corpus/sdgi_clean.jsonl")
OUTPUT_CSV = Path("2_data/1a_preprocessed_multilabel/sdgi_corpus/sdgi_clean.csv")
