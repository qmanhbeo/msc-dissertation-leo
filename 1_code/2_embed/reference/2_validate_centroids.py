...
def main() -> None:
    ...
    if args.bootstrap and args.bootstrap > 0:
        ...
        # ---- Bootstrap CI table for Appendix A.2 (canonical column only) ----
        a2_tables_dir = Path(args.output_dir) / "appendix" / "a2_bootstrap_validation_ci" / "tables"
        a2_tables_dir.mkdir(parents=True, exist_ok=True)
        ci_lines = [
            "% Auto-generated by 1_code/2_embed/reference/2_validate_centroids.py — do not edit manually",
            r"\begin{tabular}{lccc}",
            r"\toprule",
            r"SDG & F1 & 95\% CI low & 95\% CI high \\",
            r"\midrule",
        ]
        sdg_names_short = {
        ...
        for sdg in labels_17:
            ci = boot["per_sdg_f1_ci"][str(sdg)]
            ci_lines.append(
                rf"SDG {sdg} ({sdg_names_short[sdg]}) & {ci['point']:.3f} & {ci['ci_low']:.3f} & {ci['ci_high']:.3f} \\"
            )
        mf = boot["macro_f1_ci"]
        ci_lines.append(r"\midrule")
        ci_lines.append(rf"Macro-F1 (mean) & {mf['point']:.3f} & {mf['ci_low']:.3f} & {mf['ci_high']:.3f} \\")
        ci_lines.append(r"\bottomrule")
        ci_lines.append(r"\end{tabular}")
        (a2_tables_dir / "tab_validation_ci.tex").write_text("\n".join(ci_lines) + "\n", encoding="utf-8")
        log.info("Saved: %s", a2_tables_dir / "tab_validation_ci.tex")
    ...
    # tab_validation.tex — generated by 3_generate_cross_sensitivity_table.py (7-column multi-source layout)
