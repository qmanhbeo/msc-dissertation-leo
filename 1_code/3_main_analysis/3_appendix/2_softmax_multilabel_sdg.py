...
Run from project root:
    python 1_code/3_main_analysis/3_appendix/2_softmax_multilabel_sdg.py
"""
...
def metric_value(summary_rows: list[dict], variant: str, temperature: float, metric: str) -> float:
    for row in summary_rows:
        if row["variant"] == variant and float(row["temperature"]) == float(temperature) and row["metric"] == metric:
            return float(row["value"])
    raise KeyError(f"Missing summary metric: {variant=} {temperature=} {metric=}")


def write_num_tex(path: Path, summary_rows: list[dict]) -> None:
    raw_low = 0.03
    raw_high = 0.20
    cal_temps = sorted({float(r["temperature"]) for r in summary_rows if r["variant"] == "corpus_calibrated_softmax"})

    def format_range(metric: str) -> tuple[float, float]:
        values = [metric_value(summary_rows, "corpus_calibrated_softmax", temp, metric) for temp in cal_temps]
        return min(values), max(values)

    cal_cov_spear_min, cal_cov_spear_max = format_range("spearman_hard_vs_soft_coverage_gap")
    cal_sem_spear_min, cal_sem_spear_max = format_range("spearman_hard_vs_soft_semantic_gap")
    cal_cov_mad_min, cal_cov_mad_max = format_range("mean_abs_coverage_difference")
    cal_sem_mad_min, cal_sem_mad_max = format_range("mean_abs_semantic_gap_difference")

    lines = [
        "% Auto-generated by 1_code/3_main_analysis/3_appendix/2_softmax_multilabel_sdg.py — do not edit manually",
        rf"\newcommand{{\SoftmaxRawLowTemp}}{{{raw_low:.2f}}}",
        rf"\newcommand{{\SoftmaxRawHighTemp}}{{{raw_high:.2f}}}",
        rf"\newcommand{{\SoftmaxRawLowCoverageSpearman}}{{{metric_value(summary_rows, 'raw_softmax', raw_low, 'spearman_hard_vs_soft_coverage_gap'):.3f}}}",
        rf"\newcommand{{\SoftmaxRawLowSemanticSpearman}}{{{metric_value(summary_rows, 'raw_softmax', raw_low, 'spearman_hard_vs_soft_semantic_gap'):.3f}}}",
        rf"\newcommand{{\SoftmaxRawLowCoverageMad}}{{{metric_value(summary_rows, 'raw_softmax', raw_low, 'mean_abs_coverage_difference'):.3f}}}",
        rf"\newcommand{{\SoftmaxRawLowSemanticMad}}{{{metric_value(summary_rows, 'raw_softmax', raw_low, 'mean_abs_semantic_gap_difference'):.3f}}}",
        rf"\newcommand{{\SoftmaxRawHighCoverageSpearman}}{{{metric_value(summary_rows, 'raw_softmax', raw_high, 'spearman_hard_vs_soft_coverage_gap'):.3f}}}",
        rf"\newcommand{{\SoftmaxRawHighSemanticSpearman}}{{{metric_value(summary_rows, 'raw_softmax', raw_high, 'spearman_hard_vs_soft_semantic_gap'):.3f}}}",
        rf"\newcommand{{\SoftmaxRawHighCoverageMad}}{{{metric_value(summary_rows, 'raw_softmax', raw_high, 'mean_abs_coverage_difference'):.3f}}}",
        rf"\newcommand{{\SoftmaxRawHighSemanticMad}}{{{metric_value(summary_rows, 'raw_softmax', raw_high, 'mean_abs_semantic_gap_difference'):.3f}}}",
        rf"\newcommand{{\SoftmaxCalCoverageSpearmanMin}}{{{cal_cov_spear_min:.3f}}}",
        rf"\newcommand{{\SoftmaxCalCoverageSpearmanMax}}{{{cal_cov_spear_max:.3f}}}",
        rf"\newcommand{{\SoftmaxCalSemanticSpearmanMin}}{{{cal_sem_spear_min:.3f}}}",
        rf"\newcommand{{\SoftmaxCalSemanticSpearmanMax}}{{{cal_sem_spear_max:.3f}}}",
        rf"\newcommand{{\SoftmaxCalCoverageMadMin}}{{{cal_cov_mad_min:.3f}}}",
        rf"\newcommand{{\SoftmaxCalCoverageMadMax}}{{{cal_cov_mad_max:.3f}}}",
        rf"\newcommand{{\SoftmaxCalSemanticMadMin}}{{{cal_sem_mad_min:.3f}}}",
        rf"\newcommand{{\SoftmaxCalSemanticMadMax}}{{{cal_sem_mad_max:.3f}}}",
    ]
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def write_summary_tex(path: Path, summary_rows: list[dict]) -> None:
    temps = [0.03, 0.05, 0.10, 0.20]
    rows = [
        ("raw_softmax", temp)
        for temp in temps
    ] + [
        ("corpus_calibrated_softmax", temp)
        for temp in temps
    ]
    lines = [
        "% Auto-generated by 1_code/3_main_analysis/3_appendix/2_softmax_multilabel_sdg.py — do not edit manually",
        ...
        r"\toprule",
        r"Variant & $T$ & $\rho$ cov. & $\rho$ sem. & MAD cov. & MAD sem. & Top-3 sem. overlap \\",
        r"\midrule",
    ]
    for variant, temp in rows:
        label = "Raw softmax" if variant == "raw_softmax" else "Calibrated softmax"
        lines.append(
            f"{label} & {temp:.2f} & "
            f"{metric_value(summary_rows, variant, temp, 'spearman_hard_vs_soft_coverage_gap'):.3f} & "
            f"{metric_value(summary_rows, variant, temp, 'spearman_hard_vs_soft_semantic_gap'):.3f} & "
            f"{metric_value(summary_rows, variant, temp, 'mean_abs_coverage_difference'):.3f} & "
            f"{metric_value(summary_rows, variant, temp, 'mean_abs_semantic_gap_difference'):.3f} & "
            f"{int(metric_value(summary_rows, variant, temp, 'top3_overlap_semantic_gap'))}/3 \\\\"
        )
...
def main() -> None:
    ...
    write_num_tex(num_tex_path, summary_rows)
    write_summary_tex(summary_tex_path, summary_rows)
    ...
    log.info("Saved: %s", num_tex_path)
    log.info("Saved: %s", summary_tex_path)
