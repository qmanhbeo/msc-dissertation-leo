...
def _write_tex(path: Path, configs: list[dict]) -> None:
    lines = [
        ...
        r"\toprule",
        r" & \multicolumn{2}{c}{Raw gap} & \multicolumn{2}{c}{Adj.\ gap} & \multicolumn{2}{c}{Register} \\",
        r" & $\rho$ & $p$ & $\rho$ & $p$ & $\rho$ & $p$ \\",
        r"\midrule",
    ]
    ...
    for c in configs:
        row = [
            c["label"],
            _fmt(c["rho_cov_raw"]),
            _fmt_p(c["rho_cov_raw"]),
            _fmt(c["rho_cov_adj"]),
            _fmt_p(c["rho_cov_adj"]),
            _fmt(c["rho_cov_register"]),
            _fmt_p(c["rho_cov_register"]),
        ]
