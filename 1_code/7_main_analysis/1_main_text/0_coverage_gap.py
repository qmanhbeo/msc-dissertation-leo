...
Outputs:
  4_outputs/main/data/coverage_document_weighted.json              per-SDG coverage profiles + gap (canonical analysis)
  4_outputs/main/data/coverage_diagnostic_unweighted.json          segment-level (unweighted) profiles + gap (diagnostic)
...
def _route_coverage_payload(model: str, route: str) -> dict | None:
    ...
    if route == "mlp":
        ...
        gap_path = output_dir_for_model(model) / "data" / "semantic_gap_distances_zeroshot.json"
...
def run(args: argparse.Namespace) -> None:
    ...
    out_cov_gap = Path(args.out_data_dir).joinpath("coverage_document_weighted.json") if args.out_data_dir else layout.data_dir / "coverage_document_weighted.json"
    out_cov_gap_raw = Path(args.out_data_dir).joinpath("coverage_diagnostic_unweighted.json") if args.out_data_dir else layout.data_dir / "coverage_diagnostic_unweighted.json"
    out_cov_gap_mlp = layout.data_dir / "coverage_document_weighted_mlp.json"
    out_cov_gap_zs = layout.data_dir / "coverage_document_weighted_zeroshot.json"
    ...
    research = aggregate_research_scores(PAPER_SCORES_MANIFEST, scored_dir, unit="document")
    ...
    # The research corpus is segmented with the same token-aware segmenter as policy, so one
    # abstract can yield several rows. `aggregate_research_scores(unit="document")` collapses each
    # abstract to the mean of its segment score vectors (hard assignment = argmax of that mean),
    # making the research profile genuinely document-weighted and symmetric with the policy profile.
    # (Segments-per-abstract is ~1.22, so the effect is small but material — see coverage_gap.json.)
    log.info("")
    ...
    coverage_gap_out = {
        ...
        "n_research_papers": int(research["n_papers"]),
        "n_research_abstracts": int(research["n_papers"]),
        "n_research_segments": int(research["n_segments"]),
        "n_research_preprocessed": n_preprocessed,
        "n_research_dropped_at_segmentation": n_preprocessed - int(research["n_papers"]),
    ...
    coverage_gap_raw_out = {
        ...
        "note": (
            ...
            "Use coverage_document_weighted.json (document-weighted) for primary analysis."
        ),
        "n_research_papers": int(research["n_papers"]),
        "n_research_abstracts": int(research["n_papers"]),
        "n_research_segments": int(research["n_segments"]),
    ...
    # num2_coverage_gap.tex — macro definitions
    n_abstracts = int(research["n_papers"])
    n_segments = int(research["n_segments"])
    n_dropped = n_preprocessed - n_abstracts
    n_multi = int(research["papers_multi_segment"])
    seg_per_abstract = n_segments / max(n_abstracts, 1)
    n_policy = int(policy_scores.shape[0])
    num_lines = [
        ...
        rf"\newcommand{{\NResearchPreprocessed}}{{{latex_int(n_preprocessed)}}}",
        rf"\newcommand{{\NResearchDroppedAtSegmentation}}{{{latex_int(n_dropped)}}}",
        ...
        rf"\newcommand{{\NPolicySegments}}{{{latex_int(n_policy)}}}",
        ...
        rf"\newcommand{{\TotalCoverageGap}}{{{float(gap_dw.sum()) * 100:.1f}}}",
        rf"\newcommand{{\MeanCoverageGap}}{{{float(gap_dw.mean()) * 100:.1f}}}",
    ...
    for i in range(N_SDG):
        ...
        num_lines.extend([
            ...
            rf"\newcommand{{\CoverageGapSdg{word}}}{{{g_pp:.1f}}}",
        ])
    (gen_dir / "num2_coverage_gap.tex").write_text("\n".join(num_lines) + "\n", encoding="utf-8")
    log.info("Saved: %s", gen_dir / "num2_coverage_gap.tex")

    # tab2_coverage_gap.tex — full tabular block
    tab_lines = [
        "% Auto-generated by 1_code/7_main_analysis/1_main_text/0_coverage_gap.py — do not edit manually",
        r"\begin{tabular}{llrrr}",
        r"\toprule",
        r"SDG & Description & Research \% & Policy \% & Gap (pp) \\",
        r"\midrule",
    ]
    for i in range(N_SDG):
        sdg = i + 1
        ...
        r_pct = float(res_hard[i]) * 100
        p_pct = float(pol_dw_hard[i]) * 100
        g     = float(gap_dw[i]) * 100.0
        tab_lines.append(
            rf"{sdg:2d} & {name}{footnote} & {r_pct:.1f} & {p_pct:.1f} & {g:.1f} \\"
        )
    tab_lines.extend([
        r"\midrule",
    ...
    (gen_dir / "tab2_coverage_gap.tex").write_text("\n".join(tab_lines) + "\n", encoding="utf-8")
    log.info("Saved: %s", gen_dir / "tab2_coverage_gap.tex")
