...
def write_encoder_axis_semantic():
    ...
    if scibert_stats:
        ...
        lines.append(rf"\newcommand{{\SciBERTMeanGap}}{{{sg:.3f}}}")
        lines.append(rf"\newcommand{{\SciBERTMeanCos}}{{{sc:.3f}}}")
        if src is not None:
            lines.append(rf"\newcommand{{\SciBERTResCohesion}}{{{src:.3f}}}")
        if spc is not None:
            lines.append(rf"\newcommand{{\SciBERTPolCohesion}}{{{spc:.3f}}}")
    mpnet_stats = load_mean_gap_and_cohesion("all-mpnet-base-v2")
    if mpnet_stats:
        mg, mc, mrc, mpc = mpnet_stats
        lines.append(rf"\newcommand{{\MPNetMeanGap}}{{{mg:.3f}}}")
        lines.append(rf"\newcommand{{\MPNetMeanCos}}{{{mc:.3f}}}")
        if mrc is not None:
            lines.append(rf"\newcommand{{\MPNetResCohesion}}{{{mrc:.3f}}}")
        if mpc is not None:
            lines.append(rf"\newcommand{{\MPNetPolCohesion}}{{{mpc:.3f}}}")
...
# ---------------------------------------------------------------------------
# Write tab1_classifier_performance.tex (LR / MLP / ZS test F1 columns + n)
# ---------------------------------------------------------------------------
def write_validation_table(mlp_per_sdg, zs_per_sdg, test_counts):
    lines = [
        "% Auto-generated by 1_code/7_main_analysis/1_main_text/3_generate_cross_sensitivity_table.py — do not edit manually",
        r"\begin{tabular}{lcccc}",
        r"\toprule",
        r"SDG & n (test) & LR test F1 & MLP test F1 & ZS test F1 \\",
        r"\midrule",
    ]
    for sdg in range(1, N_SDG + 1):
        f1 = lr_per_sdg[sdg]
        ...
        lines.append(f"{sdg} ({sname}) & {test_counts[sdg]} & {f1:.3f} & {mf1:.3f} & {zf1:.3f} \\\\")
...
def write_cross_sensitivity():
    ...
    concept_sem_rho = f"{concept_rho_val:.3f}" if not np.isnan(concept_rho_val) else "--"
    ...
    if baseline:
        ...
        for (col_label, ranks, _), key in zip(all_cols, all_col_keys):
            ...
            if len(cv) >= 2 and len(cv) == len(bv):
                ...
                rho = float("nan")
            rho_by_col[key] = rho
            rho_cells.append(f"{rho:.3f}" if not np.isnan(rho) else "--")
    else:
        for _ in all_cols:
            rho_cells.append("--")
    tex.append(r"\midrule")
...
def write_coverage_table():
    ...
    concept_rho = f"{concept_rho_val:.3f}" if not np.isnan(concept_rho_val) else "--"
    lines = [
        f"% Auto-generated by 1_code/7_main_analysis/1_main_text/3_generate_cross_sensitivity_table.py — do not edit manually",
...
def run(args: argparse.Namespace) -> None:
    ...
    # Per-SDG held-out test counts (single source of truth: the gold test labels).
    # The test set is single-label, so these partition the 9,338 test papers and
    # are identical for every classifier evaluated on this split.
    _res_dir = model_results_dir_for_model(model)
    _test_labels = np.load(_res_dir / "labels.npy").astype(np.float32)
    _test_idx = np.load(_res_dir / "indices" / "test.npy")
    _test_lab = _test_labels[_test_idx]
    test_counts = {sdg + 1: int(_test_lab[:, sdg].sum()) for sdg in range(N_SDG)}
    ...
    write_validation_table(mlp_per_sdg, zs_per_sdg, test_counts)
