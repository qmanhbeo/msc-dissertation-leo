"""
ARCHIVED: Embedding-Model Sensitivity Comparison (MiniLM vs MPNet)

Demoted on 2026-07-26 because the project now uses only all-mpnet-base-v2.
Re-embedding the full 3.1M-paper research corpus with a second model
(e.g. all-MiniLM-L6-v2) would cost ~20 GB additional download/storage
and the dimensionality gap (768-d vs 384-d) would confound any
architecture comparison. The sample-stability and cross-sensitivity
checks already bound robustness under the chosen encoder.

The comparison script hardcoded MiniLM as "canonical" and MPNet as
"comparison", but after the default model switched to MPNet, it
compared MPNet against itself — producing meaningless results.

See dissertation.tex Appendix D (\\label{app:model-sensitivity})
for the published statement that this check was not performed.
---
...
CANONICAL_OUTPUT = DEFAULT_OUTPUT_ROOT / "main" / "data"
