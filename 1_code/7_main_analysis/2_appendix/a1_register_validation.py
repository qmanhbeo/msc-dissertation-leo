...
FEATURE_DISPLAY = [
    # (key, display name, is_words_unit, direction, load_bearing)
    ("hedge_rate", "Hedge rate (/1k words)", False, "R $>$ P", "Yes"),
    ("deontic_rate", "Deontic (must/should) rate (/1k words)", False, "P $>$ R", "Yes"),
    ("passive_rate", "Passive-voice rate (/1k words)", False, "R $>$ P", "Yes"),
    ("mean_sent_len", "Mean sentence length (words)", True, "P $>$ R", "No"),
    ("first_person_rate", "First-person rate (/1k words)", False, "$\\approx$", "No"),
    ("nominal_rate", "Nominalisation rate (/1k words)", False, "$\\approx$", "No"),
]


def write_table_sample_design(path: Path, orig: dict, opp: dict, n_per_sdg: int) -> None:
    n_res = 17 * n_per_sdg
    rows = [
        ("Original (per-SDG dedup)", orig["n"], n_res, n_res,
         orig["distinct_parents"], orig["multi_parent_units"],
         orig["multi_parent_units"], orig["mega_sdg_count"]),
        ("One-per-parent (global dedup)", opp["n"], n_res, n_res,
         opp["distinct_parents"], opp["multi_parent_units"], 0, 0),
    ]
    lines = [
        "% Auto-generated by 1_code/7_main_analysis/2_appendix/a1_register_validation.py — do not edit manually",
        r"\begin{tabular}{lrrrrrrr}",
        r"\toprule",
        r"Design & Segments & Research & Policy & Distinct docs & Multi-parent units & Mega-doc segments & Mega-SDGs \\",
        r"\midrule",
    ]
    for name, n, r, p, d, m, ms, msd in rows:
        lines.append(f"{name} & {n} & {r} & {p} & {d} & {m} & {ms} & {msd} \\\\")
    lines.extend([r"\bottomrule", r"\end{tabular}"])
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def write_table_features(path: Path, corpus_means: dict, mega_info: dict) -> None:
    mega = mega_info["mega"]
    non_mega = mega_info["non_mega"]

    def vals(k, words):
        if words:
            return (f"{corpus_means[k]['policy']:.1f}", f"{corpus_means[k]['research']:.1f}",
                    f"{mega[k]:.1f}", f"{non_mega[k]:.1f}")
        return (f"{corpus_means[k]['policy']:.2f}", f"{corpus_means[k]['research']:.2f}",
                f"{mega[k]:.2f}", f"{non_mega[k]:.2f}")

    lines = [
        "% Auto-generated by 1_code/7_main_analysis/2_appendix/a1_register_validation.py — do not edit manually",
        r"\begin{tabular}{l r r l l r r}",
        r"\toprule",
        r"Feature & \multicolumn{2}{c}{Corpus mean} & Direction & Load-bearing? & \multicolumn{2}{c}{Mega-doc vs rest} \\",
        r"\cmidrule(lr){2-3} \cmidrule(lr){6-7}",
        r"& Policy & Research & & & Mega-doc & Rest \\",
        r"\midrule",
    ]
    for key, disp, words, direction, load in FEATURE_DISPLAY:
        pol, res, meg, rest = vals(key, words)
        lines.append(f"{disp} & {pol} & {res} & {direction} & {load} & {meg} & {rest} \\\\")
    lines.extend([r"\bottomrule", r"\end{tabular}"])
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def run(args: argparse.Namespace) -> None:
    ...
    orig_si = results["item1_sample_construction"]["orig_2A"]
    opp_si = results["item1_sample_construction"]["opp_2B"]
