...
TERMS_PER_SIDE_FOCAL = 8
TERMS_PER_SIDE_FULL = 5
EXAMPLES_PER_SIDE = 3
MIN_WORDS = 30
RANDOM_SEED = 42
...
SDG_LABELS = {n: f"SDG {n}" for n in range(1, 18)}
...
def top_terms_for_sdg(research_texts: list[str], policy_texts: list[str], n_terms: int = TERMS_PER_SIDE_FOCAL) -> tuple[list[str], list[str]]:
...
def run(args: argparse.Namespace) -> None:
    ...
    n_max = max(TERMS_PER_SIDE_FOCAL, TERMS_PER_SIDE_FULL)
    all_terms: dict[int, tuple[list[str], list[str]]] = {}
    for sdg in ALL_TARGET_SDGS:
        all_terms[sdg] = top_terms_for_sdg(
            research_samples[sdg], policy_samples[sdg], n_terms=n_max
        )
    ...
    for sdg in TARGET_SDGS:
        ...
        focal_rows.append({
            ...
            "research_terms": ", ".join(rt[:TERMS_PER_SIDE_FOCAL]),
            "policy_terms": ", ".join(pt[:TERMS_PER_SIDE_FOCAL]),
    ...
    for sdg in ALL_TARGET_SDGS:
        ...
        csv_rows.append({
            ...
            "research_terms": ", ".join(rt[:TERMS_PER_SIDE_FULL]),
            "policy_terms": ", ".join(pt[:TERMS_PER_SIDE_FULL]),
            "interpretive_reading": INTERPRETIVE_READINGS.get(sdg, ""),
        })
    ...
    summary = {
        ...
        "all_sdgs": list(ALL_TARGET_SDGS),
        "random_seed": args.seed,
        "sample_per_side": args.sample_per_side,
        "terms_per_side_focal": TERMS_PER_SIDE_FOCAL,
        "terms_per_side_full": TERMS_PER_SIDE_FULL,
    ...
    write_table(tables_dir / TABLE_TEX, focal_rows)
    write_full_table(tables_dir / TABLE_TEX_FULL, full_rows)
