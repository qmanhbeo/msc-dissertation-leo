...
from model_utils import DEFAULT_EMBED_MODEL, DEFAULT_OUTPUT_ROOT, N_SDG, embed_dir_for_model, embed_research_dir_for_model, output_dir_for_model, scored_dir_for_model, resolve_model_alias
...
def load_policy_state(canonical_data_dir: Path, policy_emb_path: Path, policy_ids_path: Path, policy_scores_path: Path, *, mean_paper_top_vs_osdg: float, G: np.ndarray | None = None) -> dict[str, Any]:
    ...
    *,
    ...
    if not skip_tex:
        ...
        tab_lines = [
            ...
            r"\toprule",
            r"Sample Size & Coverage gap (vs policy) & Semantic gap (raw) & Semantic gap (adjusted) \\",
            r"\midrule",
        ]
        for row in summary_rows:
...
def run(args: argparse.Namespace) -> None:
    ...
    policy_state = load_policy_state(
        output_dir_for_model(args.embed_model, root=Path(args.output_dir)) / "data",
        _POLICY_EMB,
        _POLICY_IDS,
        _POLICY_SCORES,
