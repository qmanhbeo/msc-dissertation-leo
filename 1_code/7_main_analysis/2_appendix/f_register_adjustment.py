...
def iterative_register_check(
    ...
    # Find ρ at iteration 15 (or closest)
    rho_at_15 = next((r.get("rho_vs_iter1", 0) for r in iteration_results if r["k"] == 15), 0)
    # Plateau ρ: average of last 5 displayed iterations' ρ
    displayed = [r for r in iteration_results if r.get("rho_vs_iter1") is not None]
    plateau_rho = np.mean([r["rho_vs_iter1"] for r in displayed[-5:]]) if len(displayed) >= 5 else rho_iter1_final
    num_lines = [
