"""
Appendix G: distributional distance metrics for the research-policy semantic gap.
...
Outputs (4_outputs/appendix/{model}/g_distributional_gap/):
  ...
  tables/tab_distributional_gap.tex         per-SDG comparison table
"""
...
AUTOGEN_HEADER = (
    "% Auto-generated by 1_code/7_main_analysis/2_appendix/g_distributional_gap.py"
    " — do not edit manually"
)
# Table columns: method key -> LaTeX column header (order preserved).
TABLE_COLUMNS = [
...
def write_tables(tables_dir: Path, summary: dict[str, Any]) -> None:
    ...
    active_cols = [(m, h) for m, h in TABLE_COLUMNS if m in methods]
    tab_lines = [
        AUTOGEN_HEADER,
    ...
    tab_lines.extend([" & ".join(rho_cells) + r" \\", r"\bottomrule", r"\end{tabular}"])
...
def run(args: argparse.Namespace) -> None:
    ...
    layout = ensure_dissertation_outputs(
        output_dir, subdir="appendix/g_distributional_gap", model=model
    )
