...
def _write_table(path, rows, fmt, n_enc, has_concept, label):
    n_data = 1 + n_enc * 3 + (1 if has_concept else 0)
    ...
    h2 = [""]
    for label, _, _ in ENCODERS:
        h2.append(f"\\multicolumn{{3}}{{c}}{{{label}}}")
    if has_concept:
        h2.append("Concept")
...
def run(args):
    ...
    col_order = [
        ...
        "MiniLM LR", "MiniLM MLP", "MiniLM ZS",
        "SciBERT LR", "SciBERT MLP", "SciBERT ZS",
    ]
