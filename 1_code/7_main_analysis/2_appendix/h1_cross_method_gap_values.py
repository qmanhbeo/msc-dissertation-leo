...
def _concept_mlp_gaps(root, m):
    """Concept-retrieved MLP semantic gap (raw). Same JSON schema as the LR file."""
...
def _concept_mlp_covgaps(root, m):
    """Concept-retrieved MLP coverage gap.

    Mirrors ``load_concept_mlp_covgaps`` in 3_generate_cross_sensitivity_table.py:
    MLP-scored research coverage from ``mlp_scores_concept`` against the
    document-weighted policy profile under the same scores (there is no cached
    ``concept/coverage_document_weighted_mlp.json`` payload).
    """
    scored_dir = scored_dir_for_model(m)
...
# Concept retrieval (MPNet) carries LR (canonical classifier) + MLP (supervised
# robustness check — per AGENTS.md, MLP spans axes; only zero-shot is restricted).
ENC_METHODS = {
    "MPNet": ["LR", "MLP", "ZS"],
    "MiniLM": ["LR", "MLP"],
    "SciBERT": ["LR", "MLP"],
...
def _write_table(path, rows, fmt, concept_methods, label):
    # HEADER/DATA COUPLING: n_data (and the header rows below) come from the
    # STATIC ENC_METHODS + concept_methods, while data rows carry only the
    # columns whose loader returned values (avail_cov/avail_gap in run()). A
    # missing upstream gap JSON does NOT fail here — it silently shifts every
    # later cell left under the fixed header (trailing cells render "--").
    # Check column alignment whenever a gap JSON is unexpectedly absent.
    n_concept = len(concept_methods)
    n_data = 1 + sum(len(methods) for _, methods in ENC_METHODS.items()) + n_concept
    ...
    if n_concept:
        if n_concept > 1:
            h2.append(f"\\multicolumn{{{n_concept}}}{{c}}{{Concept}}")
        else:
            h2.append("Concept")
    ...
    for enc_label, _, _ in ENCODERS:
        ...
        start += n
    if n_concept:
        cmidrules.append(f"\\cmidrule(lr){{{start}-{start + n_concept - 1}}}")
...
def run(args):
    ...
    SCRIPT_VERSION = "3"  # v3: concept-retrieval LR + MLP columns (2026-08-28)
    ...
    fp_paths += [
        ...
        # Concept-MLP columns (v3): raw MLP gaps + the scored MLP research
        # coverage / policy scores that feed the concept-MLP coverage gap.
        concept_root / "semantic_gap_distances_mlp.json",
        scored_dir_for_model(DEFAULT_EMBED_MODEL) / "mlp_scores_concept" / "mlp_summary.json",
        scored_dir_for_model(DEFAULT_EMBED_MODEL) / "mlp_scores_concept" / "mlp_policy_scores.npy",
    ]
    ...
    # Concept retrieval (default encoder): LR (canonical classifier) + MLP
    # (supervised robustness check — AGENTS.md scope: MLP spans axes, zero-shot
    # does not).
    concept_cov_by_m = {
        "LR": _concept_covgaps(root, "all-mpnet-base-v2"),
        "MLP": _concept_mlp_covgaps(root, "all-mpnet-base-v2"),
    }
    concept_gap_by_m = {
        "LR": _concept_gaps(root, "all-mpnet-base-v2"),
        "MLP": _concept_mlp_gaps(root, "all-mpnet-base-v2"),
    }
    cov_methods = [m for m in ("LR", "MLP") if concept_cov_by_m[m] is not None]
    gap_methods = [m for m in ("LR", "MLP") if concept_gap_by_m[m] is not None]
    if cov_methods != gap_methods:
        raise RuntimeError(
            "h1: concept-retrieval availability mismatch between coverage "
            f"({cov_methods}) and semantic gap ({gap_methods}) loaders"
        )
    concept_methods = cov_methods
    ...
    if avail_cov and avail_gap:
        ...
        for sdg in all_sdgs:
            ...
            cc_vals = tuple(
                (concept_cov_by_m[m].get(sdg) if concept_cov_by_m[m] else None)
                for m in concept_methods
            )
            cg_vals = tuple(
                (concept_gap_by_m[m].get(sdg) if concept_gap_by_m[m] else None)
                for m in concept_methods
            )
            cov_rows.append(
                (sdg, cov_vals,
                 tuple(v * 100.0 if v is not None else None for v in cc_vals))
            )
            sem_rows.append((sdg, gap_vals, cg_vals))
        n_enc = len(ENCODERS)
        _write_table(
            out_dir / "tab_app_cross_method_covgap.tex",
            cov_rows, "{:.1f}", concept_methods,
            "Coverage gap (\\%)",
        )
        _write_table(
            out_dir / "tab_app_cross_method_semgap.tex",
            sem_rows, "{:.3f}", concept_methods,
            "Semantic gap (cosine)",
        )
