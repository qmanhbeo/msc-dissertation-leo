...
keyword retrieval only:
  ...
  - Research corpus: per-paper LR-vs-ZS assignment agreement, per-SDG
    agreement rates, and a 17x17 confusion matrix (row = LR assignment,
    column = ZS assignment). MLP research per-shard scores are not persisted,
    so MLP-vs-ZS is reported for policy only (research MLP agreement is
    proxied in the main text by the LR-vs-MLP gap-rank correlation).
...
def run(args: argparse.Namespace) -> None:
    ...
    for row in per_sdg_rows:
        ...
        rows_tex.append(
            ...
            f"{row['research_n_lr']:,} & {fmt_pct(row['research_agree_rate'])} & "
    ...
    lines = [
        ...
        r"\toprule",
        r"& \multicolumn{2}{c}{Research (papers)} & \multicolumn{2}{c}{Policy (segments)} & \multicolumn{3}{c}{Semantic-gap rank} \\",
        r"\cmidrule(lr){2-3} \cmidrule(lr){4-5} \cmidrule(lr){6-8}",
        r"SDG & $n_{\text{LR}}$ & Agree \% & $n_{\text{LR}}$ & Agree \% & LR & ZS & $|\Delta|$ \\",
        r"\midrule",
    ] + rows_tex + [
        r"\midrule",
