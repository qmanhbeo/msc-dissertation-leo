...
| LR + MLP grid-search orchestrator | `1_code/4_supervised_model_train/2_grid_search.py` |
| Shared training utilities | `1_code/4_supervised_model_train/train_models_utils.py` |
| Retrain script | `1_code/4_supervised_model_train/3_retrain_full_data.py` |
...
- `1_train_models_LR.py` (now folded into `train_models_utils.py` + `2_grid_search.py`): Grid `C × l1_ratio × class_weight` (8 configs). Solver `saga` for L1 compatibility. Per-class F1 per fold. Uses shared `append_grid_log()` from `model_utils.py`.
- `1_train_models_MLP.py` (now folded into `train_models_utils.py` + `2_grid_search.py`): Per-config `append_grid_log()`. No grid changes.
...
- `3_retrain_full_data.py`: Modified to support `--classifier-type {mlp,lr}`.
