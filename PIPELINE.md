...
This is a **reproducibility package**, not a from-scratch build. In normal use you do **not** run the live fetchers — you hydrate a frozen snapshot and replay it deterministically.

1. `git clone https://github.com/qmanhbeo/msc-dissertation-leo.git && cd msc-dissertation-leo`
2. `conda env create -f environment.yml` (creates the `dissertation-leo` env) → `conda activate dissertation-leo`
3. `python main.py --fetch-data-snapshot embedded` — hydrate the frozen, pre-processed snapshot into `2_data/` (offline; no OpenAlex key needed)
4. `python main.py --warm-replay-without-appendix --overwrite` — deterministically regenerate **all three encoder tracks** (MiniLM + SciBERT first, MPNet last). Add `--appendix-all` for appendix analyses, or use per-spec `--appendix-*` flags.
5. `python main.py --build-pdf --overwrite` (requires bash/Linux) or `--build-word --overwrite` → `4_outputs/dissertation.pdf` / `4_outputs/dissertation.docx`

Notes: live `--stage fetch` and `--cold-replay` are opt-in full rebuilds that need `.env` with `OPENALEX_MAILTO` + `OPENALEX_API_KEY` and take ~1 week (OpenAlex/policy URLs drift). `python main.py` with no flags is read-only status only. The stage-by-stage detail below describes the pipeline *architecture*; stages 1–3 are satisfied by the snapshot in the normal path above.
...
> In the normal path (§ Getting started), stages 1–3 are satisfied by `--fetch-data-snapshot` (raw or embedded); the live fetchers below are only the from-scratch route.
...
- preprocess policy (scraped + manual) — single `0_preprocess_policy.py`
- preprocess ungdc — `0_preprocess_ungdc_sdg.py`
- preprocess osdg — `0_preprocess_osdg.py`
- preprocess sdg benchmark — `0_preprocess_sdg_benchmark.py`
- preprocess sdg knowledge hub — `0_preprocess_sdg_knowledge_hub.py`
- preprocess aurora — `0_preprocess_aurora.py`
- preprocess sdgi — `0_preprocess_sdgi_unified.py`
...
- build research 100k-paper subset (S1; 122,472 rows — used by MiniLM/SciBERT to keep embedding feasible)
...
- embed research shards (full corpus for MPNet; `--corpus research_subset` for MiniLM/SciBERT)
...
Register adjustment is a **core main-pipeline stage**, not a sensitivity check. It runs per-encoder *inside the per-model loop* (not a single cross-encoder pass): `register_adjust.py` materialises the INLP projection matrix `G` (stored in `2_data/`, never `4_outputs/`), then the decomposition + convergence diagnostic is generated by `g_register_decomposition.py` (canon, MPNet; runs for every encoder) which also emits the iterative convergence tables consumed by Appendix E.
...
| `0_pca_register_before_after.py` | MPNet only | PCA before/after register removal (referenced in the linear flow below) |
...
| `g_distributional_h1_correlation.py` | opt-in (G1) | H1a–H1d correlation grid under distribution-aware gaps; requires the G distributional summary; run before `--build-pdf` |
...
| `c1_subset_balanced_stability.py` | C.1 | `appendix/[model]/c1_subset_balanced_stability/` |
...
| `i1_assignment_method_comparison.py` | I.1 | `appendix/[model]/i1_assignment_method_comparison/` |
| `j1_raw_value_correlation.py` | J.1 | `appendix/[model]/j1_raw_value_correlation/` |
| `k1_regression_semantic_gap.py` | K.1 | `appendix/[model]/k1_regression_semantic_gap/` |
| `g_distributional_h1_correlation.py` | G1 (opt-in) | `1_main_text/`; H1a–H1d grid under distribution-aware gaps; run before `--build-pdf` |

Note: some helper scripts in `2_appendix/` (`f2_export_register_cross_config.py`, `f3_register_iterative_cross_table.py`, `export_corpus_provenance.py`, `export_policy_subcorpus_counts.py`) are invoked *internally* by the spec'd stages above and have no standalone `--appendix-*` flag — they are not meant to be run directly.

---

## Resolved items / decisions
...
- **MLP champion `lr` (resolved):** the retrained artifact (`mlp_retrained.joblib` / `model_config.json`) and the script's argparse default now use `lr=3e-4`, matching the dissertation text. No outstanding discrepancy.
