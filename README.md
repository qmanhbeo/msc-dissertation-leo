...
## Bibliography Management and Paper Organization

### Generated citation indices

The dissertation bibliography (`writing/references.bib`) has been extracted and indexed for reference:

| Output file | Format | Contents | Generated by |
|-------------|--------|----------|--------------|
| `data/cited_papers.json` | JSON | All 22 cited papers with metadata (DOI, authors, year, journal, links) | `code/fetch_cited_papers.py` |
| `data/cited_papers.md` | Markdown | Human-readable index of papers grouped by type (articles, conference papers, reports); includes DOI and arXiv links where available | `code/fetch_cited_papers.py` |

**To regenerate these indices:**
```bash
python code/fetch_cited_papers.py
# Input:  writing/references.bib
# Output: data/cited_papers.json, data/cited_papers.md
# Note:   automatically extracts DOI URLs and arXiv links; requires bibtexparser
```

### Marker tool (PDF-to-Markdown conversion)

The [marker](https://github.com/datalab-to/marker) tool has been cloned to `marker/` for potential future use in converting PDF papers to structured Markdown. This enables:
- High-fidelity PDF → Markdown conversion (preserves layout, figures, citations)
- Batch processing of papers fetched by DOI
- Integration with embeddings pipeline if full-text analysis is needed

**Note:** Currently not used in the main pipeline (dissertation uses abstracts only). Useful if expanding to full-text semantic analysis.

**To use marker on a PDF:**
```bash
cd marker
python convert_single.py path/to/paper.pdf
# Outputs: path/to/paper.md (and extracted figures in subdirs)
```
