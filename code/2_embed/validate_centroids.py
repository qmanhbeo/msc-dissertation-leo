...
Note on expected accuracy:
  SDGs have substantial semantic overlap. A same-SDG text pair scored 0.281 cosine sim on
  average vs 0.305 for cross-SDG pairs in the OSDG sanity check (see embeddings.py). Perfect
  separation is not expected. The centroid captures the *modal direction* of each SDG cluster,
...
def main() -> None:
    ...
    num_lines = [
        "% Auto-generated by code/2_embed/validate_centroids.py — do not edit manually",
