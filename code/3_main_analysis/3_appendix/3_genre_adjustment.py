...
DEFAULT_CACHE_ROOT = Path("data/3_scored/genre_adjustment_cache")
EMBED_MANIFEST = Path("data/2_embedded/research_shards/metadata/manifest.json")
SCORE_MANIFEST = Path("data/3_scored/paper_scores_shards/metadata/manifest.json")
TEXT_MANIFEST = Path("data/1_preprocessed/research_corpus/metadata/manifest.json")
...
def ensure_genre_robustness_outputs(output_dir: Path) -> DissertationOutputs:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--skip-genre-confidence-checks",
        action="store_true",
        help="Skip the additional genre-confidence robustness checks inside this stage.",
    )
    p.add_argument(
        "--method",
        choices=["sdg_balanced", "within_sdg", "both"],
        default=DEFAULT_SDG_GENRE_METHOD,
        help="Run SDG-aware genre robustness methods: one global SDG-balanced classifier, within-SDG classifiers, or both.",
    )
    p.add_argument(
        "--random-seed",
        type=int,
        default=None,
        help="Optional override seed for the SDG-aware genre robustness methods. Defaults to --seed.",
    )
    p.add_argument(
        "--samples-per-cell",
        type=int,
        default=None,
        help="Optional cap for the number of research and policy observations sampled per SDG x genre cell.",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help="Held-out test fraction for the SDG-aware genre robustness classifiers.",
    )
    p.add_argument(
        "--classifier-type",
        choices=["logistic_regression_liblinear", "logistic_regression_saga"],
        default=DEFAULT_SDG_GENRE_CLASSIFIER_TYPE,
        help="Simple linear classifier to use for the SDG-aware genre robustness methods.",
    )
    ...
    for emb_shard, score_shard, text_shard in zip(emb_shards, score_shards, text_shards):
        ...
        shards.append(
            ResearchShard(
                ...
                start=offset,
                stop=offset + rows,
                emb_path=resolve_manifest_path(emb_shard["embedding_path"], "data/2_embedded/"),
                score_path=resolve_manifest_path(score_shard["score_path"], "data/3_scored/"),
                score_ids_path=resolve_manifest_path(score_shard["ids_path"], "data/3_scored/"),
                text_path=resolve_manifest_path(text_shard["data_path"], "data/1_preprocessed/"),
            )
        )
        offset += rows
...
def load_or_build_classifier_cache(
    *,
    ...
    if cache_path.exists():
        ...
        genre_unit = np.array(cached["genre_unit"], dtype=np.float32)
        intercept = float(cached["intercept"])
        return payload, cached["candidate_rows"], cached["metrics_payload"], coef, genre_unit, intercept
    ...
    coef_norm = float(np.linalg.norm(coef))
    if coef_norm < 1e-8:
        raise RuntimeError("Final genre classifier coefficient norm is near zero.")
    genre_unit = (coef / coef_norm).astype(np.float32)
    ...
    metrics_payload = {
        ...
        "final_train_plus_validation_model": {
            ...
            "genre_unit": [round(float(v), 10) for v in genre_unit.tolist()],
        },
    }
    write_json(
        cache_path,
        {
            ...
            "genre_unit": [float(v) for v in genre_unit.tolist()],
            "intercept": intercept,
        },
    )
    return payload, candidate_rows, metrics_payload, coef, genre_unit, intercept
...
def load_or_build_adjusted_policy_cache(
    *,
    ...
    adjusted_policy_emb = project_out_direction_and_normalize(policy_emb, genre_unit)
    ensure_dir(cache_path.parent)
...
def load_or_build_adjusted_research_cache(
    *,
    ...
    centroids, counts, cohesions = build_adjusted_research_centroids(shards, genre_unit)
    ensure_dir(cache_path.parent)
    np.savez(
        cache_path,
...
def project_out_direction_and_normalize(emb: np.ndarray, genre_unit: np.ndarray) -> np.ndarray:
    if emb.ndim != 2:
        raise ValueError(f"Expected 2D embedding matrix, got shape {emb.shape}")
    proj = emb @ genre_unit
    adjusted = emb - np.outer(proj, genre_unit).astype(np.float32)
...
def build_adjusted_research_centroids(
    ...
    for shard in shards:
        ...
        adjusted = project_out_direction_and_normalize(emb, genre_unit)
        ...
        merged.append(
            {
                ...
                "genre_adjusted_similarity": adj_row["semantic_similarity"],
                "genre_adjusted_gap": adj_row["semantic_gap"],
                "delta_gap": (
                    None
                ...
                "genre_adjusted_research_cohesion": adj_row["research_cohesion"],
                "genre_adjusted_policy_cohesion": adj_row["policy_cohesion"],
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "raw_policy_cohesion",
                "genre_adjusted_similarity",
                "genre_adjusted_gap",
                "genre_adjusted_research_cohesion",
                "genre_adjusted_policy_cohesion",
                "delta_gap",
                "unreliable",
                "unreliable_reason",
            ],
    ...
    for shard in shards:
        ...
        for row_idx in range(emb.shape[0]):
            payload = {
                ...
                "genre_score": float(decision_scores[row_idx]),
                "score_type": "decision_function",
        ...
        output_rows.append(
            {
                ...
                "genre_score": round(payload["genre_score"], 6),
...
def resolve_samples_per_cell(
    *,
    ...
    if min_available is None or min_available <= 0:
        raise RuntimeError("Could not determine a positive SDG x genre cell minimum.")
    ...
    for row in cell_rows:
        row["sampled_per_genre"] = effective
...
def load_or_build_sdg_balanced_method_cache(
    *,
    ...
    coef = model.coef_[0].astype(np.float32)
    unit = normalize_unit_vector(coef, label="SDG-balanced genre direction")
    payload_out = {
        "train_size": int(X_train.shape[0]),
        "test_size": int(X_test.shape[0]),
        "metrics": metrics,
        "coef": [float(v) for v in coef.tolist()],
        "genre_unit": [float(v) for v in unit.tolist()],
...
def build_within_sdg_adjusted_research_centroids(
    *,
    ...
    if not vectors_by_sdg:
        raise RuntimeError("No within-SDG genre directions were learned.")
...
def load_or_build_within_sdg_method_cache(
    *,
    ...
    for sdg_idx in range(N_SDG):
        ...
        coef = model.coef_[0].astype(np.float32)
        unit = normalize_unit_vector(coef, label=f"Within-SDG genre direction for SDG {sdg_idx + 1}")
    ...
    if available_units:
        avg_direction = normalize_unit_vector(np.mean(np.vstack(available_units), axis=0), label="Average within-SDG genre direction")
...
def solve_regression_vectors_from_cell_stats(
    ...
    if np.any(counts <= 0):
        ...
        raise RuntimeError(
            "Regression genre decomposition requires non-empty genre x SDG cells; "
    ...
    beta_genre = (policy_means[REGRESSION_BASELINE_SDG - 1] - research_means[REGRESSION_BASELINE_SDG - 1]).astype(np.float32)
    ...
    delta = (within_raw[1:] - beta_genre[None, :]).astype(np.float32)

    global_unit = normalize_unit_vector(beta_genre, label="Regression global genre direction")
    within_units = np.vstack(
        [
            normalize_unit_vector(within_raw[sdg_idx], label=f"Regression within-SDG genre direction for SDG {sdg_idx + 1}")
            for sdg_idx in range(N_SDG)
        ]
    ).astype(np.float32)
...
def load_or_build_regression_method_cache(
    *,
    ...
    if cache_npz.exists() and cache_json.exists():
        log.info("Cache hit: regression genre vectors (%s)", key)
    ...
    log.info("Cache miss: regression genre vectors (%s)", key)
    solved = solve_regression_vectors_from_cell_stats(counts, sums)
    ensure_dir(cache_npz.parent)
    np.savez(
        cache_npz,
        global_vector=solved["beta_genre"].astype(np.float32),
    ...
    write_json(
        cache_json,
        {
            ...
            "global_vector_norm": round(float(np.linalg.norm(solved["beta_genre"])), 6),
        },
    )
    return payload, {
        "baseline_sdg": int(solved["baseline_sdg"]),
        ...
        "delta": solved["delta"].astype(np.float32),
        "beta0": solved["beta0"].astype(np.float32),
        "global_vector_norm": float(np.linalg.norm(solved["beta_genre"])),
    }
...
def plot_regression_vs_classifier_alignment(
    ...
    *,
    ...
    ax.set_title("Classifier vs regression genre-direction alignment", fontsize=8.5, loc="left")
    ax.text(
        0.01,
        0.98,
...
def write_regression_latex_outputs(
    ...
    *,
    ...
    num_lines = [
        "% Auto-generated by code/3_main_analysis/3_appendix/3_genre_adjustment.py — do not edit manually",
        rf"\newcommand{{\RegressionGenreVsClassifierCosine}}{{{float(similarity_payload['global_regression_vs_classifier_cosine']):.3f}}}",
        rf"\newcommand{{\RegressionGenreAgreementBand}}{{{similarity_payload['agreement_band'].replace('_', ' ')}}}",
        rf"\newcommand{{\RegressionGenreMeanAbsCosine}}{{{float(regression_alignment_summary['mean_absolute_cosine']):.3f}}}",
        rf"\newcommand{{\RegressionGenreMedianAbsCosine}}{{{float(regression_alignment_summary['median_absolute_cosine']):.3f}}}",
        rf"\newcommand{{\RegressionGenreMaxAbsCosine}}{{{float(regression_alignment_summary['max_absolute_cosine']):.3f}}}",
        rf"\newcommand{{\RegressionGenreStrongestSdg}}{{SDG {int(regression_alignment_summary['strongest_alignment_sdg'])}}}",
        rf"\newcommand{{\RegressionGenreWeakestSdg}}{{SDG {int(regression_alignment_summary['weakest_alignment_sdg'])}}}",
    ...
    (tables_dir / "num_regression_genre_alignment.tex").write_text("\n".join(num_lines) + "\n", encoding="utf-8")
    ...
    (tables_dir / "tab_regression_genre_alignment.tex").write_text("\n".join(table_lines) + "\n", encoding="utf-8")


def build_genre_vector_cosine_rows(
    *,
...
def load_or_build_local_adjusted_reseparability(
    *,
    ...
    X_train_adj = project_out_direction_and_normalize(X_train, genre_unit)
    X_val_adj = project_out_direction_and_normalize(X_val, genre_unit)
    X_test_adj = project_out_direction_and_normalize(X_test, genre_unit)
    X_train_val_adj = project_out_direction_and_normalize(X_train_val, genre_unit)
...
def learn_additional_directions(
    *,
    ...
    direction_rows: list[dict[str, Any]] = [
        {
            "step": 1,
            "source": "canonical_genre_direction",
        }
    ]
...
def load_or_build_local_topic_match(
    *,
    ...
    for sdg_idx in range(N_SDG):
        ...
        adjusted_research_emb = project_out_direction_and_normalize(research_emb, genre_unit)
...
def write_genre_confidence_latex_outputs(
    tables_dir: Path,
    ...
    num_lines = [
        "% Auto-generated by code/3_main_analysis/3_appendix/3_genre_adjustment.py — do not edit manually",
        rf"\newcommand{{\GenreConfidenceAdjustedResepRawRocAuc}}{{{float(raw_test['roc_auc']):.3f}}}",
        rf"\newcommand{{\GenreConfidenceAdjustedResepAdjustedRocAuc}}{{{float(adjusted_test['roc_auc']):.3f}}}",
        rf"\newcommand{{\GenreConfidenceAdjustedResepDeltaRocAuc}}{{{float(summary['adjusted_reseparability']['relative_roc_auc_drop']):.3f}}}",
        rf"\newcommand{{\GenreConfidenceAdjustedResepAdjustedAccuracy}}{{{float(adjusted_test['accuracy']):.3f}}}",
        rf"\newcommand{{\GenreConfidenceHeldoutMeanRocAuc}}{{{float(held_mean['test_roc_auc']):.3f}}}",
        rf"\newcommand{{\GenreConfidenceHeldoutMeanFOne}}{{{float(held_mean['test_f1']):.3f}}}",
        rf"\newcommand{{\GenreConfidenceHeldoutMinRocAuc}}{{{min_held_roc_auc:.3f}}}",
        rf"\newcommand{{\GenreConfidenceHeldoutMaxRocAuc}}{{{max_held_roc_auc:.3f}}}",
        rf"\newcommand{{\GenreConfidenceMultiKOne}}{{{int(k1_row['k'])}}}",
        rf"\newcommand{{\GenreConfidenceMultiKMax}}{{{int(kmax_row['k'])}}}",
        rf"\newcommand{{\GenreConfidenceMultiKOneMeanGap}}{{{float(k1_row['mean_adjusted_gap']):.3f}}}",
        rf"\newcommand{{\GenreConfidenceMultiKMaxMeanGap}}{{{float(kmax_row['mean_adjusted_gap']):.3f}}}",
        rf"\newcommand{{\GenreConfidenceMultiKOneToMaxDelta}}{{{float(kmax_row['mean_adjusted_gap']) - float(k1_row['mean_adjusted_gap']):.3f}}}",
        rf"\newcommand{{\GenreConfidenceMultiKMaxResidualRocAuc}}{{{float(kmax_row['test_roc_auc']):.3f}}}",
        rf"\newcommand{{\GenreConfidenceStableTopGapSdgs}}{{{format_sdg_label_list(top_stable)}}}",
        rf"\newcommand{{\GenreConfidenceTopicRawMatchedGap}}{{{float(topic_overall['raw_mean_matched_gap']):.3f}}}",
        rf"\newcommand{{\GenreConfidenceTopicAdjustedMatchedGap}}{{{float(topic_overall['adjusted_mean_matched_gap']):.3f}}}",
        rf"\newcommand{{\GenreConfidenceTopicDeltaMatchedGap}}{{{float(topic_overall['adjusted_mean_matched_gap']) - float(topic_overall['raw_mean_matched_gap']):.3f}}}",
    ]
    (tables_dir / "num_genre_confidence_checks.tex").write_text("\n".join(num_lines) + "\n", encoding="utf-8")
    ...
    tab_lines = [
        ...
        r"\midrule",
        r"Adjusted re-separability & Raw ROC-AUC \GenreConfidenceAdjustedResepRawRocAuc; adjusted \GenreConfidenceAdjustedResepAdjustedRocAuc ($\Delta$ \GenreConfidenceAdjustedResepDeltaRocAuc) & One removed direction leaves cross-corpus separability very high. \\",
        r"Held-out SDG generalization & Mean held-out ROC-AUC \GenreConfidenceHeldoutMeanRocAuc; mean F1 \GenreConfidenceHeldoutMeanFOne{} & The learned genre axis transfers across unseen SDG blocks, so it is not merely within-SDG topic leakage. \\",
        r"Multi-direction subtraction & Mean adjusted gap \GenreConfidenceMultiKOneMeanGap{} at $k=\GenreConfidenceMultiKOne$ and \GenreConfidenceMultiKMaxMeanGap{} at $k=\GenreConfidenceMultiKMax$ & Additional discriminative directions continue to compress gap magnitude, although the largest-gap SDGs remain concentrated in \GenreConfidenceStableTopGapSdgs. \\",
        r"Topic-matched nearest neighbor & Mean matched gap \GenreConfidenceTopicRawMatchedGap{} raw and \GenreConfidenceTopicAdjustedMatchedGap{} adjusted ($\Delta$ \GenreConfidenceTopicDeltaMatchedGap) & Within-SDG nearest cross-corpus matches remain far apart after subtraction and become slightly less similar on average. \\",
        r"\bottomrule",
        r"\end{tabular}",
    ]
    (tables_dir / "tab_genre_confidence_checks.tex").write_text("\n".join(tab_lines) + "\n", encoding="utf-8")


def plot_genre_confidence_curve(figures_dir: Path, multi_direction: dict[str, Any]) -> None:
    ...
    ax_top.set_title(
        "Additional genre-confidence checks: multi-direction subtraction sensitivity",
        fontsize=8.5,
        loc="left",
    )
    ...
    fig.tight_layout()
    fig.savefig(figures_dir / "fig_genre_confidence_curve.pdf", bbox_inches="tight")
    fig.savefig(figures_dir / "fig_genre_confidence_curve.png", bbox_inches="tight", dpi=150)
    plt.close(fig)
...
def plot_sdg_genre_robustness_comparison(
    *,
    output_dir: Path,
    ...
    fig.savefig(manuscript_figures_dir / "fig_sdg_genre_robustness_comparison.pdf", bbox_inches="tight")
    fig.savefig(manuscript_figures_dir / "fig_sdg_genre_robustness_comparison.png", bbox_inches="tight", dpi=150)
    plt.close(fig)
...
def plot_within_sdg_vector_similarity_heatmap(
    ...
    *,
    ...
    ax.set_xlabel("SDG")
    ax.set_ylabel("SDG")
    ax.set_title("Within-SDG genre-direction cosine similarity", fontsize=8.5, loc="left")
...
def write_sdg_genre_adjustment_note(
    path: Path,
    *,
    method: str,
    seed: int,
    ...
    note = [
        ...
        "3. `regression` treats each embedding coordinate as an outcome and estimates genre, SDG, and genre x SDG effects by exact OLS cell contrasts.",
        "",
        "How to run:",
        f"- `python code/3_main_analysis/3_appendix/3_genre_adjustment.py --method {method}`",
        "",
...
def write_sdg_genre_latex_outputs(
    tables_dir: Path,
    *,
    ...
    num_lines = [
        "% Auto-generated by code/3_main_analysis/3_appendix/3_genre_adjustment.py — do not edit manually",
        rf"\newcommand{{\SdgGenreBalancedAccuracy}}{{{balanced_accuracy}}}",
        rf"\newcommand{{\SdgGenreBalancedMacroFOne}}{{{balanced_macro_f1}}}",
        rf"\newcommand{{\SdgGenreBalancedRocAuc}}{{{balanced_roc_auc}}}",
        rf"\newcommand{{\SdgGenreBalancedMeanGap}}{{{'' if mean_balanced_gap is None else f'{mean_balanced_gap:.3f}'}}}",
        rf"\newcommand{{\SdgGenreWithinMeanAccuracy}}{{{within_mean_accuracy}}}",
        rf"\newcommand{{\SdgGenreWithinMeanMacroFOne}}{{{within_mean_macro_f1}}}",
        rf"\newcommand{{\SdgGenreWithinMeanRocAuc}}{{{within_mean_roc_auc}}}",
        rf"\newcommand{{\SdgGenreWithinMeanGap}}{{{'' if mean_within_gap is None else f'{mean_within_gap:.3f}'}}}",
        rf"\newcommand{{\SdgGenreRawMeanGap}}{{{'' if mean_raw_gap is None else f'{mean_raw_gap:.3f}'}}}",
        rf"\newcommand{{\SdgGenreWithinFittedCount}}{{{len(available_within)}}}",
        rf"\newcommand{{\SdgGenreWithinSkippedCount}}{{{len(skipped_within)}}}",
        rf"\newcommand{{\SdgGenreGlobalVsAverageCosine}}{{{'' if global_vs_avg is None else f'{float(global_vs_avg):.3f}'}}}",
    ]
    (tables_dir / "num_sdg_genre_robustness.tex").write_text("\n".join(num_lines) + "\n", encoding="utf-8")
    ...
    if sdg_balanced_metrics is not None:
        tab_lines.append(
            r"SDG-balanced global classifier & Test accuracy \SdgGenreBalancedAccuracy; macro-F1 \SdgGenreBalancedMacroFOne; ROC-AUC \SdgGenreBalancedRocAuc; mean adjusted gap \SdgGenreBalancedMeanGap & One global research-policy direction is learned after equalising the SDG composition of the training sample. \\"
        )
    if within_metrics_rows is not None:
        tab_lines.append(
            r"Within-SDG classifiers & Mean test accuracy \SdgGenreWithinMeanAccuracy; mean macro-F1 \SdgGenreWithinMeanMacroFOne; mean ROC-AUC \SdgGenreWithinMeanRocAuc; mean adjusted gap \SdgGenreWithinMeanGap & SDG-specific genre directions are fit where sufficient data exist (\SdgGenreWithinFittedCount{} SDGs fit, \SdgGenreWithinSkippedCount{} skipped). \\"
        )
    if cosine_rows is not None:
        tab_lines.append(
            r"Direction stability & Global-vs-average cosine \SdgGenreGlobalVsAverageCosine & Higher cosine similarity means the within-SDG genre directions point in roughly the same direction as the SDG-balanced global control. \\"
        )
    tab_lines.extend([r"\bottomrule", r"\end{tabular}"])
    (tables_dir / "tab_sdg_genre_robustness.tex").write_text("\n".join(tab_lines) + "\n", encoding="utf-8")
...
def load_or_build_genre_projection_interpretability_cache(
    *,
    cache_root: Path,
    base_signature: dict[str, Any],
    ...
    payload = {
        "schema_version": CACHE_SCHEMA_VERSION,
        "phase": "genre_projection_interpretability",
        "base_signature": base_signature,
    ...
    key = stable_cache_key(payload)
    cache_npz = cache_npz_path(cache_root, "genre_projection_interpretability", key)
    cache_json = cache_json_path(cache_root, "genre_projection_interpretability", key)
    if cache_npz.exists() and cache_json.exists():
        log.info("Cache hit: genre projection interpretability (%s)", key)
        cached = np.load(cache_npz)
        meta = load_json(cache_json)
        return payload, {
    ...
    log.info("Cache miss: genre projection interpretability (%s)", key)
    policy_scores = (policy_emb @ genre_unit).astype(np.float32)
    ...
    for shard in shards:
        emb = np.load(shard.emb_path).astype(np.float32)
        scores = (emb @ genre_unit).astype(np.float32)
...
def build_genre_projection_summary_rows(
    *,
...
def write_genre_direction_interpretation_report(
    path: Path,
    *,
    ...
    lines = [
        "# Genre Direction Interpretation",
        "",
        "This file is an automatic manual-inspection aid. It exposes raw examples aligned with the learned global genre direction.",
        "",
        f"- Vector source: `{vector_path}`",
        f"- Projection score: `dot(x, g)` where `g` is the SDG-balanced global genre unit vector",
...
def plot_genre_projection_distribution(
    figures_dir: Path,
    *,
    research_scores: np.ndarray,
    research_sdgs: np.ndarray,
    ...
    ax_hist.set_title("Projection onto the SDG-balanced global genre direction", fontsize=8.5, loc="left")
    ...
    fig.tight_layout()
    fig.savefig(figures_dir / "fig_genre_projection_distribution.pdf", bbox_inches="tight")
    fig.savefig(figures_dir / "fig_genre_projection_distribution.png", bbox_inches="tight", dpi=150)
    plt.close(fig)


def load_or_build_genre_sdg_alignment_cache(
    *,
    cache_root: Path,
    base_signature: dict[str, Any],
    sdg_balanced_method_signature: dict[str, Any],
    ...
    payload = {
        "schema_version": CACHE_SCHEMA_VERSION,
        "phase": "genre_sdg_alignment",
        "base_signature": base_signature,
        "sdg_balanced_method_signature": sdg_balanced_method_signature,
        "has_within_vectors": bool(within_vectors is not None),
    }
    key = stable_cache_key(payload)
    cache_path = cache_json_path(cache_root, "genre_sdg_alignment", key)
    if cache_path.exists():
        log.info("Cache hit: genre-vs-SDG alignment (%s)", key)
        return payload, load_json(cache_path)

    log.info("Cache miss: genre-vs-SDG alignment (%s)", key)
...
def plot_genre_sdg_alignment(figures_dir: Path, alignment_rows: list[dict[str, Any]]) -> None:
    ...
    ax.set_xticks(xs)
    ax.set_xlabel("SDG")
    ax.set_ylabel("Cosine similarity with global genre direction")
    ax.set_title("Alignment between the SDG-balanced genre direction and SDG centroids", fontsize=8.5, loc="left")
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    fig.tight_layout()
    fig.savefig(figures_dir / "fig_genre_sdg_alignment.pdf", bbox_inches="tight")
    fig.savefig(figures_dir / "fig_genre_sdg_alignment.png", bbox_inches="tight", dpi=150)
    plt.close(fig)


def write_genre_interpretability_latex_outputs(
    tables_dir: Path,
    *,
    alignment_rows: list[dict[str, Any]],
    ...
    num_lines = [
        "% Auto-generated by code/3_main_analysis/3_appendix/3_genre_adjustment.py — do not edit manually",
        rf"\newcommand{{\GenreProjectionPolicyTopMean}}{{{policy_mean:.3f}}}",
        rf"\newcommand{{\GenreProjectionResearchBottomMean}}{{{research_mean:.3f}}}",
        rf"\newcommand{{\GenreAlignmentMeanAbsCosine}}{{{float(alignment_summary['mean_absolute_cosine']):.3f}}}",
        rf"\newcommand{{\GenreAlignmentMedianAbsCosine}}{{{float(alignment_summary['median_absolute_cosine']):.3f}}}",
        rf"\newcommand{{\GenreAlignmentMaxAbsCosine}}{{{float(alignment_summary['max_absolute_cosine']):.3f}}}",
        rf"\newcommand{{\GenreAlignmentStrongestSdg}}{{SDG {int(alignment_summary['strongest_alignment_sdg'])}}}",
        rf"\newcommand{{\GenreAlignmentWeakestSdg}}{{SDG {int(alignment_summary['weakest_alignment_sdg'])}}}",
    ]
    (tables_dir / "num_genre_interpretability.tex").write_text("\n".join(num_lines) + "\n", encoding="utf-8")
    ...
    (tables_dir / "tab_genre_sdg_alignment.tex").write_text("\n".join(align_lines) + "\n", encoding="utf-8")
    ...
    (tables_dir / "tab_genre_projection_examples.tex").write_text("\n".join(example_lines) + "\n", encoding="utf-8")
...
def write_latex_outputs(
    ...
    valid_adj = [float(row["genre_adjusted_gap"]) for row in merged_rows if row["genre_adjusted_gap"] is not None]
    ...
    largest_adj_row = max(
        (row for row in merged_rows if row["genre_adjusted_gap"] is not None),
        key=lambda row: float(row["genre_adjusted_gap"]),
    )
    ...
    num_lines = [
        "% Auto-generated by code/3_main_analysis/3_appendix/3_genre_adjustment.py — do not edit manually",
        rf"\newcommand{{\GenreClassifierSamplePerClass}}{{{sample_balance['research']:,}}}",
        rf"\newcommand{{\GenreClassifierTrainN}}{{{split_sizes['train']}}}",
        rf"\newcommand{{\GenreClassifierValN}}{{{split_sizes['validation']}}}",
        rf"\newcommand{{\GenreClassifierTestN}}{{{split_sizes['test']}}}",
        rf"\newcommand{{\GenreClassifierSelectedC}}{{{selected['C']}}}",
        rf"\newcommand{{\GenreClassifierTestAccuracy}}{{{test_metrics['accuracy']:.3f}}}",
        rf"\newcommand{{\GenreClassifierTestRocAuc}}{{{test_metrics['roc_auc']:.3f}}}",
        rf"\newcommand{{\GenreClassifierTestFOne}}{{{test_metrics['f1']:.3f}}}",
        rf"\newcommand{{\GenreClassifierTestPrecision}}{{{test_metrics['precision']:.3f}}}",
        rf"\newcommand{{\GenreClassifierTestRecall}}{{{test_metrics['recall']:.3f}}}",
        ...
        rf"\newcommand{{\LargestAdjustedGapValue}}{{{float(largest_adj_row['genre_adjusted_gap']):.3f}}}",
    ]
    for sdg_num, word in num_words.items():
        ...
        if row["genre_adjusted_gap"] is not None:
            num_lines.append(
                rf"\newcommand{{\AdjustedSemanticGapSdg{word}}}{{{float(row['genre_adjusted_gap']):.3f}}}"
            )
        if row["delta_gap"] is not None:
            num_lines.append(rf"\newcommand{{\SemanticGapDeltaSdg{word}}}{{{float(row['delta_gap']):.3f}}}")
    (tables_dir / "num_genre_adjustment.tex").write_text("\n".join(num_lines) + "\n", encoding="utf-8")
    ...
    for row in sorted_rows:
        tab_lines.append(
            ...
            rf"{float(row['genre_adjusted_gap']):.3f} & {float(row['delta_gap']):.3f} & "
    ...
    (tables_dir / "tab_genre_adjusted_semgap.tex").write_text(
        "\n".join(tab_lines) + "\n",
        encoding="utf-8",
    )
...
def plot_gap_comparison(figures_dir: Path, merged_rows: list[dict[str, Any]]) -> None:
    valid_rows = [row for row in merged_rows if row["genre_adjusted_gap"] is not None]
    if not valid_rows:
        raise RuntimeError("No finite genre-adjusted semantic-gap rows available for plotting.")

    names, _ = build_sdg_name_maps()
    valid_rows = sorted(valid_rows, key=lambda row: float(row["genre_adjusted_gap"]), reverse=True)
    y = np.arange(len(valid_rows))
    height = 0.36
    ...
    ax.barh(
        y + height / 2,
        [float(row["genre_adjusted_gap"]) for row in valid_rows],
        height=height,
        color="#2166AC",
        alpha=0.9,
        label="Genre-adjusted semantic gap",
    )
    ax.set_yticks(y)
    ax.set_yticklabels([f"SDG {row['sdg']} {names[row['sdg']]}" for row in valid_rows], fontsize=7.5)
    ...
    fig.tight_layout()
    fig.savefig(figures_dir / "fig_genre_adjusted_semantic_gap_comparison.pdf", bbox_inches="tight")
    fig.savefig(
        figures_dir / "fig_genre_adjusted_semantic_gap_comparison.png",
        bbox_inches="tight",
        dpi=150,
    )
    plt.close(fig)
...
def main() -> None:
    ...
    out_combined_csv = layout.root / "genre_adjusted_semantic_gaps.csv"
    out_metrics = layout.root / "genre_adjustment_classifier_metrics.json"
    ...
    if missing:
        raise FileNotFoundError("Genre-adjustment stage is missing required inputs: " + ", ".join(missing))
    base_signature = {
        "required_inputs": cache_input_signatures(required_paths),
        "cache_schema_version": CACHE_SCHEMA_VERSION,
        "script": "code/3_main_analysis/3_appendix/3_genre_adjustment.py",
    }
    ...
    classifier_signature, candidate_rows, metrics_payload, coef, genre_unit, intercept = load_or_build_classifier_cache(
        cache_root=cache_root,
        sample_signature=sample_signature,
        X_train=X_train,
        y_train=y_train,
    ...
    combined_payload = {
        "robustness_check": "genre_adjusted_semantic_gap",
    ...
    log.info(
        "SDG-balanced sample plan: %d per SDG x genre cell",
        int(sdg_balanced_sample["effective_samples_per_cell"]),
    )
    sdg_balanced_X, sdg_balanced_y, sdg_balanced_strata = assemble_sdg_balanced_dataset(
        shards=shards,
        policy_emb=policy_emb,
    ...
    sdg_balanced_unit = np.array(sdg_balanced_method["genre_unit"], dtype=np.float32)

    if args.method in ("sdg_balanced", "both"):
        sdg_balanced_metrics_out = {
            "method": "sdg_balanced",
            "random_seed": genre_methods_seed,
            "classifier_type": args.classifier_type,
            "requested_samples_per_cell": args.samples_per_cell,
        ...
        write_json(genre_adjustment_dir / "sdg_balanced_metrics.json", sdg_balanced_metrics_out)
        np.save(genre_adjustment_dir / "sdg_balanced_genre_vector.npy", sdg_balanced_unit.astype(np.float32))
        ...
        sdg_balanced_results = compute_sdg_semantic_gaps(
            ...
            policy_ids=policy_score_ids,
            chunk_cap=args.chunk_cap,
            rng=np.random.default_rng(genre_methods_seed),
        )
        ...
        write_gap_comparison_csv(
            genre_adjustment_dir / "sdg_balanced_gap_comparison.csv",
            sdg_balanced_rows,
            adjusted_similarity_field="sdg_balanced_adjusted_similarity",
            adjusted_gap_field="sdg_balanced_adjusted_gap",
        )
        plot_sdg_balanced_gap_comparison(genre_adjustment_dir, sdg_balanced_rows)
    ...
    if args.method in ("within_sdg", "both"):
        ...
        cosine_rows, cosine_global_vs_average = build_genre_vector_cosine_rows(
        ...
        write_rows_csv(
            genre_adjustment_dir / "within_sdg_metrics.csv",
            [
                "sdg",
                "available_research",
                "available_policy",
        ...
        np.save(genre_adjustment_dir / "within_sdg_genre_vectors.npy", within_vectors.astype(np.float32))
        write_rows_csv(
            genre_adjustment_dir / "genre_vector_cosine_similarity.csv",
            ["comparison_type", "left", "right", "cosine_similarity"],
            cosine_rows,
        )
        plot_within_sdg_vector_similarity_heatmap(
            genre_adjustment_dir,
            within_vectors=within_vectors,
            available_sdgs=available_sdgs,
        )
        ...
        write_gap_comparison_csv(
            genre_adjustment_dir / "within_sdg_gap_comparison.csv",
            within_sdg_rows,
            adjusted_similarity_field="within_sdg_adjusted_similarity",
            adjusted_gap_field="within_sdg_adjusted_gap",
        )
    ...
    write_json(
        genre_adjustment_dir / "summary.json",
        {
            "method": args.method,
            "random_seed": genre_methods_seed,
            "classifier_type": args.classifier_type,
            ...
            "projection_report_file": "genre_direction_interpretation.md",
            "genre_sdg_alignment_file": "genre_sdg_alignment.csv",
        },
    )
    write_sdg_genre_latex_outputs(
        layout.tables_dir,
    ...
    plot_sdg_genre_robustness_comparison(
        output_dir=genre_adjustment_dir,
        manuscript_figures_dir=layout.figures_dir,
        sdg_balanced_rows=sdg_balanced_rows,
        within_sdg_rows=within_sdg_rows,
    )

    projection_signature, projection_artifacts = load_or_build_genre_projection_interpretability_cache(
        cache_root=cache_root,
        base_signature=base_signature,
        sdg_balanced_method_signature=sdg_balanced_method_signature,
        shards=shards,
    ...
    top_policy_like_path = genre_adjustment_dir / "top_policy_like_texts.csv"
    top_research_like_path = genre_adjustment_dir / "top_research_like_texts.csv"
    genre_projection_summary_path = genre_adjustment_dir / "genre_projection_summary.csv"
    genre_direction_report_path = genre_adjustment_dir / "genre_direction_interpretation.md"
    write_rows_csv(
    ...
    projection_summary_rows = build_genre_projection_summary_rows(
    ...
    write_genre_direction_interpretation_report(
        genre_direction_report_path,
        report_policy_rows=projection_artifacts["report_policy_rows"],
        report_research_rows=projection_artifacts["report_research_rows"],
        vector_path=genre_adjustment_dir / "sdg_balanced_genre_vector.npy",
    )
    plot_genre_projection_distribution(
        layout.figures_dir,
        research_scores=projection_artifacts["research_scores"],
        research_sdgs=projection_artifacts["research_sdgs"],
        policy_scores=projection_artifacts["policy_scores"],
    ...
    if args.method in ("within_sdg", "both"):
        ...
        within_vectors_path = genre_adjustment_dir / "within_sdg_genre_vectors.npy"
    ...
    alignment_signature, alignment_artifacts = load_or_build_genre_sdg_alignment_cache(
        cache_root=cache_root,
        base_signature=base_signature,
        sdg_balanced_method_signature=sdg_balanced_method_signature,
    ...
    genre_sdg_alignment_path = genre_adjustment_dir / "genre_sdg_alignment.csv"
    genre_sdg_alignment_summary_path = genre_adjustment_dir / "genre_sdg_alignment_summary.json"
    within_sdg_genre_centroid_alignment_path = genre_adjustment_dir / "within_sdg_genre_centroid_alignment.csv"
    write_rows_csv(
        genre_sdg_alignment_path,
        ["sdg", "cosine_similarity", "abs_cosine_similarity"],
        alignment_artifacts["alignment_rows"],
    )
    write_json(genre_sdg_alignment_summary_path, alignment_artifacts["summary"])
    write_rows_csv(
        within_sdg_genre_centroid_alignment_path,
        ["sdg", "cosine_similarity", "abs_cosine_similarity", "classifier_available"],
        alignment_artifacts["within_rows"],
    )
    plot_genre_sdg_alignment(layout.figures_dir, alignment_artifacts["alignment_rows"])
    write_genre_interpretability_latex_outputs(
        layout.tables_dir,
    ...
    regression_global_vector_path = genre_adjustment_dir / "regression_genre_vector.npy"
    regression_within_vectors_path = genre_adjustment_dir / "regression_within_sdg_genre_vectors.npy"
    regression_alignment_path = genre_adjustment_dir / "regression_genre_sdg_alignment.csv"
    regression_similarity_path = genre_adjustment_dir / "regression_vs_classifier_similarity.json"
    regression_gap_path = genre_adjustment_dir / "regression_gap_comparison.csv"
    ...
    _, regression_global_policy_emb = load_or_build_adjusted_policy_cache(
        ...
        policy_emb=policy_emb,
        genre_unit=regression_global_unit,
    )
    ...
    regression_global_results = compute_sdg_semantic_gaps(
        ...
        policy_ids=policy_score_ids,
        chunk_cap=args.chunk_cap,
        rng=np.random.default_rng(genre_methods_seed + 701),
    )
    ...
    regression_within_results = compute_sdg_semantic_gaps(
        ...
        policy_ids=policy_score_ids,
        chunk_cap=args.chunk_cap,
        rng=np.random.default_rng(genre_methods_seed + 702),
    )
    ...
    write_json(
        genre_adjustment_dir / "summary.json",
        {
            ...
            "projection_report_file": "genre_direction_interpretation.md",
            "genre_sdg_alignment_file": "genre_sdg_alignment.csv",
            "regression_alignment_file": "regression_genre_sdg_alignment.csv",
    ...
    if not args.skip_genre_confidence_checks:
        ...
        genre_confidence_artifacts["adjusted_reseparability_metrics_json"] = str(adjusted_resep_metrics_path)
        genre_confidence_artifacts["adjusted_reseparability_validation_grid_csv"] = str(adjusted_resep_grid_path)
        ...
        genre_confidence_artifacts["heldout_sdg_generalization_json"] = str(heldout_json_path)
        genre_confidence_artifacts["heldout_sdg_generalization_csv"] = str(heldout_csv_path)
        ...
        genre_confidence_artifacts["multi_direction_gap_curve_json"] = str(multi_json_path)
        genre_confidence_artifacts["multi_direction_gap_curve_csv"] = str(multi_curve_csv_path)
        genre_confidence_artifacts["multi_direction_per_sdg_csv"] = str(multi_per_sdg_csv_path)
        ...
        genre_confidence_artifacts["topic_matched_pair_summary_json"] = str(topic_json_path)
        genre_confidence_artifacts["topic_matched_pair_summary_csv"] = str(topic_csv_path)
        genre_confidence_artifacts["topic_matched_pair_examples_csv"] = str(topic_examples_csv_path)

        genre_confidence_summary = build_local_confidence_summary(
        ...
        genre_confidence_summary_path = confidence_dir / "genre_confidence_summary.json"
        write_json(genre_confidence_summary_path, genre_confidence_summary)
        genre_confidence_artifacts["genre_confidence_summary_json"] = str(genre_confidence_summary_path)

        write_genre_confidence_latex_outputs(
            layout.tables_dir,
            genre_confidence_summary,
            adjusted_reseparability,
            heldout_generalization,
            multi_direction,
            topic_match,
        )
        genre_confidence_artifacts["latex_num_tex"] = str(layout.tables_dir / "num_genre_confidence_checks.tex")
        genre_confidence_artifacts["latex_tab_tex"] = str(layout.tables_dir / "tab_genre_confidence_checks.tex")

        plot_genre_confidence_curve(layout.figures_dir, multi_direction)
        genre_confidence_artifacts["figure_pdf"] = str(layout.figures_dir / "fig_genre_confidence_curve.pdf")
        genre_confidence_artifacts["figure_png"] = str(layout.figures_dir / "fig_genre_confidence_curve.png")

        genre_confidence_manifest_path = confidence_dir / "manifest.json"
        write_json(
            genre_confidence_manifest_path,
            {
                "check_bundle": "genre_confidence_checks",
                "seed": args.seed,
                "c_grid": c_grid,
                ...
                "artifacts": genre_confidence_artifacts,
            },
        )
        genre_confidence_artifacts["manifest_json"] = str(genre_confidence_manifest_path)
        log.info("Saved genre-confidence robustness checks to %s", confidence_dir)
    ...
    write_cache_manifest(
        cache_root,
        {
            ...
            "artifacts": {
                ...
                "genre_projection_summary_csv": str(genre_projection_summary_path),
                "genre_direction_interpretation_md": str(genre_direction_report_path),
                "genre_sdg_alignment_csv": str(genre_sdg_alignment_path),
                "genre_sdg_alignment_summary_json": str(genre_sdg_alignment_summary_path),
                "within_sdg_genre_centroid_alignment_csv": str(within_sdg_genre_centroid_alignment_path),
                "regression_genre_vector_npy": str(regression_global_vector_path),
                "regression_within_sdg_genre_vectors_npy": str(regression_within_vectors_path),
                "regression_genre_sdg_alignment_csv": str(regression_alignment_path),
            ...
            "interpretability_cache_keys": {
                "genre_projection_interpretability": stable_cache_key(projection_signature),
                "genre_sdg_alignment": stable_cache_key(alignment_signature),
            },
            ...
            "genre_confidence_artifacts": genre_confidence_artifacts,
        },
    )
