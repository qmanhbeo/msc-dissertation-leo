...
For local pre-release testing with a one-off archive:
    python code/data_backup_and_fetch/fetch_data_snapshot.py \\
        --profile curated \\
...
def ensure_snapshot_available(url: str, sha256_hex: str, profile_name: str) -> None:
    if not url or not sha256_hex:
        raise RuntimeError(
            ...
            f"'{profile_name}'. Populate code/data_backup_and_fetch/data_snapshot_manifest.json with the final HTTPS "
...
def main() -> None:
    ...
    if data_root.exists():
        if not args.overwrite:
            raise RuntimeError("data/ already exists. Re-run with --overwrite to replace it.")
