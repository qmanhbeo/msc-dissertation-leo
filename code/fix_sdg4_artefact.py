...
def main():
    ...
    genuine_pct  = genuine  / n_sdg4 * 100 if n_sdg4 > 0 else 0.0
    artefact_pct = ml_artefact / n_sdg4 * 100 if n_sdg4 > 0 else 0.0
    ambig_pct    = ambig    / n_sdg4 * 100 if n_sdg4 > 0 else 0.0
    # Conservative adjusted share: treat ambiguous as artefacts removed from 22.1% total
    adjusted_share = round((genuine / n_papers) * 100) if n_papers > 0 else 0
    ...
    num_lines = [
        ...
        rf"\newcommand{{\NsdgFourTotal}}{{{_ltx_num(n_sdg4)}}}",
        rf"\newcommand{{\NsdgFourGenuineCount}}{{{_ltx_num(genuine)}}}",
        rf"\newcommand{{\NsdgFourGenuinePct}}{{{genuine_pct:.1f}}}",
        rf"\newcommand{{\NsdgFourArtefactCount}}{{{_ltx_num(ml_artefact)}}}",
        rf"\newcommand{{\NsdgFourArtefactPct}}{{{artefact_pct:.1f}}}",
        rf"\newcommand{{\NsdgFourAmbiguousCount}}{{{_ltx_num(ambig)}}}",
        rf"\newcommand{{\NsdgFourAmbiguousPct}}{{{ambig_pct:.1f}}}",
        rf"\newcommand{{\SdgFourAdjustedSharePct}}{{{adjusted_share}}}",
    ]
    num_path = gen_dir / "num_sdg4.tex"
    num_path.write_text("\n".join(num_lines) + "\n", encoding="utf-8")
    print(f"\nSaved → {num_path}")
