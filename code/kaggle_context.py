...
def main() -> None:
    ...
    mean_index_score = float(np.mean(list(sdg_means.values())))

    # num_context.tex — macro definitions
    num_lines = [
        "% Auto-generated by code/kaggle_context.py — do not edit manually",
        rf"\newcommand{{\MeanSdgIndexScore}}{{{mean_index_score:.2f}}}",
        rf"\newcommand{{\HHOneResearchVsIndexPearsonR}}{{{_fmt(h21_corr['pearson_r'])}}}",
        rf"\newcommand{{\HHOneResearchVsIndexPearsonP}}{{{h21_corr['pearson_p']:.3f}}}",
        rf"\newcommand{{\HHOneResearchVsIndexSpearmanRho}}{{{_fmt(h21_corr['spearman_rho'])}}}",
        rf"\newcommand{{\HHOneResearchVsIndexSpearmanP}}{{{h21_corr['spearman_p']:.3f}}}",
        rf"\newcommand{{\HHTwoSemanticVsIndexPearsonR}}{{{_fmt(h22_corr['pearson_r'])}}}",
        rf"\newcommand{{\HHTwoSemanticVsIndexPearsonP}}{{{h22_corr['pearson_p']:.3f}}}",
        rf"\newcommand{{\HHTwoSemanticVsIndexSpearmanRho}}{{{_fmt(h22_corr['spearman_rho'])}}}",
        rf"\newcommand{{\HHTwoSemanticVsIndexSpearmanP}}{{{h22_corr['spearman_p']:.3f}}}",
    ]
    for sdg, word in _sdg_num_words.items():
        score = sdg_means.get(sdg)
        if score is not None:
            num_lines.append(rf"\newcommand{{\Sdg{word}IndexScore}}{{{score:.2f}}}")
    (gen_dir / "num_context.tex").write_text("\n".join(num_lines) + "\n", encoding="utf-8")
    log.info("Saved: %s", gen_dir / "num_context.tex")

    # tab_sdgindex.tex — full tabular block
    tab_lines = [
        r"\begin{tabular}{lrrrr}",
        r"\toprule",
        r"Test & Pearson $r$ & $p$ & Spearman $\rho$ & $p$ \\",
        r"\midrule",
        rf"H21: Research proportion vs.\ SDG Index score"
        rf" & \HHOneResearchVsIndexPearsonR & \HHOneResearchVsIndexPearsonP"
        rf" & \HHOneResearchVsIndexSpearmanRho & \HHOneResearchVsIndexSpearmanP \\",
        rf"H22: Semantic gap vs.\ SDG Index score"
        rf" & \HHTwoSemanticVsIndexPearsonR & \HHTwoSemanticVsIndexPearsonP"
        rf" & \HHTwoSemanticVsIndexSpearmanRho & \HHTwoSemanticVsIndexSpearmanP \\",
        r"\bottomrule",
        r"\end{tabular}",
    ]
    (gen_dir / "tab_sdgindex.tex").write_text("\n".join(tab_lines) + "\n", encoding="utf-8")
    log.info("Saved: %s", gen_dir / "tab_sdgindex.tex")
