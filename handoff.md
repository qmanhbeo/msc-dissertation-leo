...
**Branch:** `register-adj` off `main` at `8a83eaf`. 10 commits, latest `aae27aa`. Not yet merged.

**Authoritative spec:** `PLAN_register_topic_decomposition.md` (597 lines). This plan documents the entire register-topic decomposition restructure — science gate, engineering tasks §6.1–§6.7, manuscript rewrites §12, architecture decisions §10.
...
| **LaTeX compilation check** | Not done | Need `latexmk`/`pdflatex`/`biber` (bash/WSL). Should verify no undefined control sequences from new macros |
| **PCA before/after figure** | Not started (§6.5.4) | Optional — the existing `0_pca_semantic_landscape.py` produces a 4-panel PCA. A register-adjusted version would project a stratified sample through G and show side-by-side raw vs adjusted. Not critical for the dissertation. |
| **h1_register_correlation_table.py** | Not started (§6.5.3) | Optional — per-config correlation table. The decomposition table (`tab_register_decomposition.tex`) and interaction extension (`4_4_interaction_extended.json`) already provide the key correlations. |
| **Interaction extension wiring** | Partial | `2_coverage_semantic_interaction.py` still produces raw-only `4_4_interaction_correlation_asymmetry.json` and `num_interaction.tex`. The new `g_interaction_extended.py` produces `4_4_interaction_extended.json` with all three correlation sets. The old script's `num_interaction.tex` is still loaded by `dissertation.tex` (provides `\HPrimary*` macros). No conflict, but the old JSON is a strict subset of the new one. |

## 5. Concerns

1. **LaTeX compilation is unverified.** The new macros (`\MeanRawGap`, `\RhoCovTopic`, `\RawGapSdgSeventeen`, etc.) are defined in `num_register_topic_decomposition.tex` which is loaded via `\InputIfFileExists`. If the file is missing at compile time, the macros are silently undefined and LaTeX will error. The file exists now but must exist at build time.

2. **`--stage analysis` now runs register_adjust for all 3 encoders.** This is correct but expensive (~10 min for MPNet). If someone runs `--stage analysis` expecting a fast re-analysis, they'll wait for INLP. This matches `--cold-replay` behavior but may surprise users.
...
**§6.5.3 — h1_register_correlation_table.py:** NOT DONE. Optional.

**§6.5.4 — PCA before/after figure:** NOT DONE. Optional.
...
The session completed two full audit passes and all fixes. The last action was committing `aae27aa` (audit pass 2 fixes). The branch is clean — no uncommitted changes, all generators verified working, all outputs present. The natural next step would be:

1. **LaTeX compilation check** — run `python main.py --build-pdf --overwrite` to verify no undefined control sequences
2. **Merge to main** — once LaTeX compiles clean
3. **Optional:** Implement §6.5.3 (h1_register_correlation_table) and §6.5.4 (PCA before/after figure) if desired

No work was interrupted — the session reached a natural stopping point with all critical and medium issues resolved.
