...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--appendix-all", action="store_true", help="Run all appendix stages (A2, A3, B2, C, C1, C0, H1, I1, F2, J1, K1) standalone (requires existing main-text outputs). D1/model-selection is excluded by design: its grid search is a parked one-off under 4_outputs/not_in_replay/.")
...
def _selected_action_flags(args: argparse.Namespace) -> list[str]:
    """Names of every action flag set on `args` (dispatch is single-action, so
    callers use this to warn about silently-ignored flags)."""
    selected = [
        name for name, on in (
            ("--warm-replay-without-appendix", args.warm_replay_without_appendix),
            ("--warm-replay-with-appendix", args.warm_replay_with_appendix),
            ("--cold-replay", args.cold_replay),
            ("--appendix-all", args.appendix_all),
            ("--fetch-data-snapshot", bool(args.fetch_data_snapshot)),
            ("--backup-data-snapshot", bool(args.backup_data_snapshot)),
            ("--fetch-encoder-models", args.fetch_encoder_models),
            ("--get-outputs-final", args.get_outputs_final),
        ) if on
    ]
    selected += [f"--{spec['flag']}" for spec in APPENDIX_SPECS
                 if getattr(args, spec["flag"].replace("-", "_"))]
    if args.stage:
        selected.append(f"--stage {args.stage}")
    return selected


def warn_ambiguous_dispatch(args: argparse.Namespace) -> None:
    """main()'s dispatch runs exactly ONE action per invocation; flag
    ...
    if len(chosen) > 1:
        print(
            ...
            + ". Only ONE action runs per invocation — all others are ignored. "
            "Run them as separate commands.",
            file=sys.stderr,
        )
    elif chosen and (args.build_pdf or args.build_word) and not (
        args.appendix_all or any(getattr(args, spec["flag"].replace("-", "_")) for spec in APPENDIX_SPECS)
    ):
        # --build-pdf/--build-word compose only with appendix actions; after a
        # replay/fetch they would be dropped, so point at the follow-up command.
        targets = " and ".join(
            flag for flag, on in (("--build-pdf", args.build_pdf), ("--build-word", args.build_word)) if on
        )
        print(
            f"[warning] {targets} was requested together with {chosen[0]}, but PDF/Word builds "
            "only run alongside appendix actions. Build afterwards with: "
            f"python main.py {targets} --overwrite",
            file=sys.stderr,
        )
...
def _run_main_analysis_steps(output_dir: Path, model: str, overwrite: bool = False, include_appendix: bool = False) -> None:
    ...
    # Model-selection grid search is a parked one-off (4_outputs/not_in_replay/
    # model_selection/); Appendix D.1 reads it directly. Not run in replay.
...
def _print_replay_deferral_notice() -> None:
    ...
    distributional gap battery (Appendix G) and the model-selection grid search
    (Appendix D.1, parked one-off). Their committed 4_outputs/ files are the
    submission state; the commands below regenerate them on demand."""
    print(
        "\nIntentionally NOT regenerated by replay (too slow for a quick warm/cold replay; the\n"
        "committed outputs are the submission state):\n"
        "  - Distributional gap tables + macros (Appendix G; tab13/tab14, num13/num18).\n"
        "    NOTE: --embeddings adjusted is REQUIRED — dissertation.tex reads the adjusted track.\n"
        "  - Model-selection grid search (Appendix D.1; parked one-off under "
        "4_outputs/not_in_replay/model_selection/).\n"
        "To refresh the distributional tables before --build-pdf:\n"
        "  python main.py --appendix-g-distributional --embeddings adjusted --overwrite\n"
        "  python main.py --appendix-g-distributional-h1 --overwrite   # needs the step above run first"
    )
...
def run_warm_replay(
    ...
    *,
    ...
    if args.embed_model != DEFAULT_EMBED_MODEL:
        print(
            f"NOTE: --embed-model {args.embed_model!r} is ignored by warm replay "
            "(all three encoder tracks are rebuilt, mirroring --cold-replay)."
        )
    ...
    scope_txt = (
        ...
        if include_appendix
        else "Main text outputs rebuilt (appendix outputs left committed)"
    )
    print(
        f"{scope_txt} for all encoder tracks. To build the dissertation PDF, run:\n"
...
def main() -> None:
    ...
    if args.stage:
        ...
        model = args.embed_model
        # Model-selection CV is a parked one-off (4_outputs/not_in_replay/
        # model_selection/); Appendix D.1 reads it directly.
